...
chrome.extension.sendMessage({
	...
	for(var i in response.blocked) {
		...
		if(re.test(window.location.href)) {
			allowed = 0;
			console.log("Link Klipper is blocked on this site: "+response.blocked[i]+"~"+window.location.href)
		}
	}
	...
	outerloop: for (var i = 0; i < page_links.length; i++) {
		...
		if (this.settings[this.setting].options.ignore.length > 1) {
			for (var k = 1; k < this.settings[this.setting].options.ignore.length; k++) {
				...
				var notFound = true;
				var enabled = this.settings[this.setting].options.enabled;
				if(enabled == 2)
				{
					if((page_links[i].innerHTML.split('.').pop() == pattern) || (page_links[i].href.split('.').pop() == pattern))
					{
...
function detech(x, y, open){
	...
	for (var i = 0; i < this.links.length; i++) {
		...
		if ((!this.smart_select || this.links[i].important) && !(this.links[i].x1 > this.box.x2 || this.links[i].x2 < this.box.x1 || this.links[i].y1 > this.box.y2 || this.links[i].y2 < this.box.y1)) {
			if (open) {
				open_tabs.push({"url": this.links[i].href, "title": this.links[i].innerText, "hostname": this.location.host})
			}
			...
			if (this.links[i].box == null) {
				...
				link_box.style.id = "linkklipper-link"; //? link_box = $('<span id="linkclump-link" />');
				link_box.style.margin = "0px auto";
				link_box.style.border = "1px solid #088A45";
